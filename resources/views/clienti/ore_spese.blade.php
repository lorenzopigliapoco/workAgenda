...
@section('content')
<div class="container">
  <h1>Riepilogo ore spese per Clienti</h1>
  <p> Compila il form, inserendo  un range temporale nel quale calcolare i costi e le ore spese. </p>

  <form action="{{ URL::action('App\Http\Controllers\ClienteController@cerca') }}" method="POST">
  {{ csrf_field() }} 
    ...
    <div class = "row">
      ...
      <div class="form-group row">
        ...
        <div class = "col-sm col-form-label text-md-right">
          ...
          <div class="col-sm">
            <select class="form-control" name="ssid_cliente" id="ssid_cliente" placeholder="ssid_cliente" required>
              <option disabled selected value> -- Scegli un'opzione -- </option>
              @foreach($clienti as $cliente)
               <option ><?=$cliente->ssid?> </option>
              @endforeach
            </select>
          </div>
        </div>
  ...
  <table class="table table-bordered table-striped">
    <thead class="thead-dark">
        <tr>
            ...
            <th scope="col" style="text-align:center">SSID Cliente </th>
        ...
        @foreach( $ore as $ora )
          <tr>
            ...
            <td style="text-align:center">{{$ora->ssid_cliente}} </td>
            <td style="text-align:center">{{$ora->progetto}} - {{$ora->nome_progetto}}</td>
